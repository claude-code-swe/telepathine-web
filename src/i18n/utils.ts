...
export function stripLocalePrefix(pathname: string): string {
  const segments = pathname.split('/').filter(Boolean);
  const localeIndex = segments.findIndex(s => locales.includes(s as Locale) && s !== defaultLocale);
  if (localeIndex >= 0) {
...
export function detectLocaleFromHeaders(headers: Headers): Locale {
  ...
  if (cfCountry && (spanishSpeakingCountries as readonly string[]).includes(cfCountry)) {
    return 'es';
  }
